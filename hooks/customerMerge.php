...
function ciniki_sms_hooks_customerMerge($ciniki, $business_id, $args) {
	...
	if( $rc['stat'] != 'ok' ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2770', 'msg'=>'Unable to find sms items', 'err'=>$rc['err']));
	}
	$items = $rc['rows'];
	foreach($items as $i => $row) {
		...
		if( $rc['stat'] != 'ok' ) {
			return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2771', 'msg'=>'Unable to update sms items.', 'err'=>$rc['err']));
		}
		$updated++;
	}
